...
bool init()
{
    ...
    if (GLEW_ARB_vertex_shader && GLEW_ARB_fragment_shader)
        ...
        printf("No GLSL! Is this Yama Buddha!");
        return false;
    }
    ...
    // Misc
    SDL_WM_SetCaption( "visualize A", NULL );
...
void draw_line(float amplitude,float factor,float deflection)
{
		
	glBegin(GL_LINES);
		glVertex2f(0.0f,0.0f);
		glVertex2f(amplitude*deflection, amplitude*factor);
	glEnd();
}
...
int main(int argc, char* argv[])
{
    ...
    if(argc > 1)
    {
        if(!strcmp(argv[1], "-r"))
        {
            printf("I am hearing you, I'll will be using default recording device.\n\n");
            record = true;
        }
        else
        {
	...
	//printf("amp_loc %f texture %f\n",amplitude_loc,texture_loc);
    // Scene
    //
    // Sphere
	...
	if (high_freq_avg > 7000)
	{ 
		high_freq_avg = 7000;
		
	}
	float amplitude = 1/(10*normalize(low_avg,low_max));
	//float high_amplitude = 1/(normalize(high_avg,high_max));
	float low_amp = low_freq_avg/100;
	//low_freq_avg = low_freq_avg/amplitude;
	//high_freq_avg = high_freq_avg/amplitude;
	float high_amp = 1/(10*normalize(high_avg,high_max));
	printf("low: %f  high: %f  midmax-mid %f amp %f low_amp %f high_amp %f\n", low_freq_avg, high_freq_avg, 1/normalize(low_avg, low_max),amplitude, low_amp/10, 1/(10*normalize(high_avg,high_max)));
	...
	glColor4f(1.f, 0.0f, 0.0f, 1.0f);
       	
//	gluSphere(quadric, 9.0f, 64, 64);
        glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0);
	...
	if(amplitude > 0.3)
        	rot_angle += 2*amplitude;

        glTranslatef(0.0f, 0.0f, -40.0f);
        glRotatef(rot_angle,0.0f,0.0f,1.0f);
        //glRotatef(20,0.0f,0.0f,1.0f);
        glColor4f(1.0f, 0.9f, amplitude, 1.0f);

	gluSphere(quadric, amplitude*8, 64, 64);
...
/*
	for(float i =0.0; i=2.0;i+=0.5)
		draw_line(amplitude,20,i);
	for(float i =2.0; i<4.0;i+=0.5)
		draw_line(amplitude,30,i);
	for(float i = 4.0 ;i<7.0; i+=0.5)
		draw_line(amplitude,-15,i);
	for(float i=7.0; i<10.0; i+=0.5)
		draw_line(amplitude,-20,i);
*/

	//draw_line(y,x)
	draw_line(high_amp,12,0.0);
	draw_line(high_amp,-12,-0.0);
	draw_line(high_amp,20,1.0);
	draw_line(high_amp,-20,-1.0);
	
	draw_line(high_amp,12,2.0);
	draw_line(high_amp,12,4.0);
	draw_line(high_amp,20,6.0);
	draw_line(high_amp,20,8.0);
	draw_line(high_amp,20,10.0);	
	draw_line(high_amp,30,12.0);
	draw_line(high_amp,30,14.0);
	draw_line(high_amp,15,16.0);
	draw_line(high_amp,15,18.0);
	draw_line(high_amp,25,20.0);
	
	draw_line(high_amp,-16,-4.0);
	draw_line(high_amp,-27,-6.0);
	draw_line(high_amp,-22,-8.0);

	draw_line(amplitude,-22,2.0);
	draw_line(amplitude,-16,4.0);
	draw_line(amplitude,-25,6.0);

	// y,-x
	draw_line(amplitude,15,-2.0);
	draw_line(amplitude,28,-4.0);
	draw_line(amplitude,19,-6.0);
	draw_line(amplitude,21,-8.0);
	draw_line(amplitude,21,-10.0);
	draw_line(amplitude,25,-12.0);

	draw_line(amplitude,-16,4.0);
	draw_line(amplitude,-27,6.0);
	draw_line(amplitude,-22,8.0);
	draw_line(amplitude,-30,10.0);
	draw_line(amplitude,-20,12.0);
	draw_line(amplitude,-13,14.0);

	draw_line(amplitude,-1,4.0);
	draw_line(amplitude,-2,6.0);
	draw_line(amplitude, 5,8.0);
	draw_line(amplitude,8,10.0);
	draw_line(amplitude,-2,12.0);
	draw_line(amplitude,-13,14.0);

	draw_line(amplitude,-1,-4.0);
	draw_line(amplitude,-2,-6.0);
	draw_line(amplitude,-5,-8.0);
	draw_line(amplitude,-8,-27.0);
	draw_line(amplitude,-2,-12.0);
	draw_line(amplitude,-13,-14.0);

	draw_line(amplitude,20,-10.0);	
	draw_line(amplitude,30,-12.0);
	draw_line(amplitude,30,-14.0);
	draw_line(amplitude,15,-16.0);
	draw_line(amplitude,15,-18.0);
	draw_line(amplitude,25,-20.0);
